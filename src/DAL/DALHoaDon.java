        ...
        if(open()){
            try{
                ...
                while(rs.next()){
                    ...
                    hd.setMaKhuyenMai(rs.getString("MA_KHYEN_MAI"));
        ...
        try{
            if(open()){
                ...
                if(rs.next()){
                    ...
                    String mkm = rs.getString("MA_KHUIYEN_MAI");
...
public boolean xoaHD(DTOHoaDon hd){
    ...
    public boolean suaHD(DTOHoaDon hd, String MaHD){
        ...
        if(open()){
            try{
                String SQL = "UPDATE HOA_DON SET MA_HOA_DON = ?, MA_NHAN_VIEN = ?, MA_KHACH_HANG = ?, MA_KHUYEN_MAI = ?, THOI_GIAN_TAO = ?, TONG_SO_LUONG = ?, TONG_GIA = ?, THANH_TIEN = ? WHERE MaHD = ? ";
                p = c.prepareStatement(SQL);
                p.setString(1, hd.getMaHoaDon());
                p.setString(2, hd.getMaNhanVien());
                p.setString(3, hd.getMaKhachHang());
